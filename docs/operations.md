...
`helix-rel-chaos-yard` is centered on reliability work in Solidity. The technical target is: Develop a Solidity command-oriented project for chaos scenarios with framed sample traffic, bounds and ordering tests, and offline replay mode.
...
The score is deterministic. The current threshold is 179. Risk has a penalty of 5, latency has a penalty of 3, and weight contributes a bonus of 2.
